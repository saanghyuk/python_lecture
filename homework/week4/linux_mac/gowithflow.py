...
def merge_and_sort(list_data_a, list_data_b):
    ...
    for i in range(len(list_data_b)):
        list_data_a.append(list_data_b[i])
        print(i)

        if i == len(list_data_b)-1:
            list_data_a.sort()
...
def comparison_list_size(list_data_a, list_data_b):
    ...
    # 한 줄 이상의 코드로 작성 가능하나,
    # 반드시 결과 값을 result 변수에 할당하여 반환
    length_a = len(list_data_a)
    length_b = len(list_data_b)
    result=[];
...
def binary_converter(decimal_number):
    ...
    while(decimal_number > 1):
        remainder = str(decimal_number % 2)
        ...
        print("===========================")
    if(decimal_number == 1):
        binary = str(decimal_number)+binary
    result = binary
...
def number_of_cases(list_data):
    ...
    for i in range(len(list_data)):
        for j in range(len(list_data)):
            element = str(list_data[i])+str(list_data[j])
...
def main():
    ...
    # print(merge_and_sort(["a", "b", "e"], ["d", "c"]))
    # print(delete_a_list_element([1,2,3,4], 4))
    # print(comparison_list_size([1,2,3,4,5,6,6,7,7], [12,3,4]))
    # print(odd_even_check(3, 5))
    # print(discount_price(40000))
    # print(find_smallest_value([1,2,3,4,5]))
    # print(binary_converter(8))
    # print(number_of_cases(['a','a', 1,2,3,4,5]))
